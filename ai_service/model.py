...
KATEGORI_MAP = {
    "Teknologi Informasi & Jaringan (IT)": "IT",
    "Kelistrikan & Tata Udara (MEP)": "MEP",
    "Infrastruktur & Sipil": "INFRA",
    "Perabotan & Inventaris (Furniture)": "PERABOTAN",
}
...
class KlasifikasiLaporan:
    ...
    def predict(self, text):
        ...
        kategori_label = self.le_kategori.classes_[idx_k]
        urgensi_label = self.le_urgensi.classes_[idx_u]

        kategori_short = KATEGORI_MAP.get(kategori_label, kategori_label)
        urgensi_upper = urgensi_label.upper()

        return {
            "kategori": kategori_short,
            "urgensi": urgensi_upper,
            "kategori_label": kategori_label,
            "urgensi_label": urgensi_label,
            "confidence_kategori": float(prob_k[idx_k]),
            "confidence_urgensi": float(prob_u[idx_u]),
            "probabilities_kategori": {
                KATEGORI_MAP.get(str(cls), str(cls)): float(p)
                for cls, p in zip(self.le_kategori.classes_, prob_k)
            },
            "probabilities_urgensi": {
                str(cls).upper(): float(p)
