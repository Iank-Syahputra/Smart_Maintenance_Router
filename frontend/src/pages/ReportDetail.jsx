...
export default function ReportDetail() {
  ...
  const [hasUpvoted, setHasUpvoted] =
    useState(false);
  ...
  const fetchReport = async () => {
    try {
      ...
      if (user && reportData.upvotes) {
        const alreadyUpvoted =
          reportData.upvotes.some(
            (upvote) =>
              upvote.userId === user.id
          );

        setHasUpvoted(alreadyUpvoted);
      }
  ...
  const handleCommentSubmit =
    async () => {
      ...
      try {
        ...
        setCommentError(
          "Gagal mengirim komentar. Pastikan Anda sudah login."
        );
          ...
          <button
            ...
            className={`flex items-center gap-2 rounded-xl px-5 py-3 font-medium transition-all duration-300 ${
              hasUpvoted
                ...
                : "border border-amber-300 bg-amber-50 text-amber-700 hover:bg-amber-400 hover:text-white"
            }`}
          >
            <ArrowBigUp size={20} />
            {report._count?.upvotes || 0}
